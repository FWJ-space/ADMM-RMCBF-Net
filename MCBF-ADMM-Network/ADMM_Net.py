...
from ZXM_layer import ZXM_layer  # 自定义层
import mbegin
import Mydataset
...
class ADMM_Net(torch.nn.Module):
    ...
    def forward(self, rho1,rho2,mu1,mu2,v1,v2,t,H1,H2,W11,W12,W21,W22,P1_out,P2_out):
        ...
        for stage in range(99):
            ...
            rho1,rho2,mu1,mu2,v1,v2,t,H1,H2,W11,W12,W21,W22 = self.ZXM_layer(rho1,rho2,mu1,mu2,v1,v2,t,H1,H2,W11,W12,W21,W22,self.c)
            ...
            loss1 = (p11+p22 - P1_out-P2_out).norm(2) / ((P1_out + P2_out)*(P1_out + P2_out))
            print('Loss: {}'.format(loss1))

        print('stage: {}'.format(stage + 2))
        # Z_layer
        print("Z_layer")
...
for epoch in range(1):
    ...
    for i in range(5):
        ...
        for rho1,rho2,mu1,mu2,v1,v2,t,H1,H2,W11,W12,W21,W22,P1_out,P2_out in train_loader:
            ...
            loss = loss_func((yp1 + yp2 ), (P1_out + P2_out))/((P1_out + P2_out)*(P1_out + P2_out))
            optimizer.zero_grad()  # 梯度置零，因为反向传播过程中梯度会累加上一次循环的梯度
            loss.backward()
            optimizer.step()
